...
for i=1:NUM_EPISODES_FOR_METRICS
    ...
    if(sum(cell_str.data(ind,3)) == 1 || sum(cell_str.data(ind,4)) == 0)
       ...
       mean_distance_closest = mean(cell_str.data(ind,12));
       distance_to_closest_obstacle = [distance_to_closest_obstacle mean_distance_closest];
    end
end
...
mean_min_distance_to_obstacles = mean(distance_to_closest_obstacle)
std_min_distance_to_obstacles = std(distance_to_closest_obstacle)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%T_gazebo_matlab = [0 -1 4;1 0 4;0 0 1];
%wall_color_3D = 'b';
%wall_color = 'b';
%wall_width = 2;

%rectangle('position',[0, 0, 0.1, 8], 'FaceColor', wall_color)
%rectangle('position',[0.1, 7.9, 7.9, 0.1], 'FaceColor', wall_color)
%rectangle('position',[8.0, 0, 0.1, 8], 'FaceColor', wall_color)
%rectangle('position',[0.1, 0, 7.9, 0.1], 'FaceColor', wall_color)
...
%obstacle_width = 0.5;
%pos_obs_gazebo_1 = [1.3 1.5 1] + [-obstacle_width/2.0 obstacle_width/2.0 0];
%pos_obs_gazebo_2 = [1.3 -1.5 1] + [-obstacle_width/2.0 obstacle_width/2.0 0];
...
%pos_x = cell_str.data(ind_plot_trajetory,4);
%pos_y = cell_str.data(ind_plot_trajetory,5);
